/**
 * Session pause/cooldown management.
 * Ported from openclaw-weixin src/api/session-guard.ts.
 *
 * When errcode -14 (session expired) is returned, pauses all API calls
 * for one hour. Uses in-memory Map; for persistence across restarts
 * in Workers, wrap with StateAdapter at the adapter layer.
 */
